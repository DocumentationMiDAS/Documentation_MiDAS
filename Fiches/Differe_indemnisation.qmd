...
ouverture_droit = PJC.groupBy("id_midas", "KROD3") \
    .agg(F.min("KDDPJ").alias("date_ouv_droit"))
```
:::
...
calcul_differe_specifique <- fait_generateur %>%
  mutate(duree_differe_specifique = case_when(passe_pro == "Licenciement_eco" ~ min(KQISLP/coeff_2022, 75)
                                              passe_pro == "Licenciement_autres" ~ min(KQISLP/coeff_2022, 150),
                                              TRUE ~ min(KQISLP/coeff_2022, 150)))
...
date_fin = date(2022, 12, 31)
coeff_2022 = 95.8

CDT = ( 
    CDT.withColumn("KCMCA_num", F.when((F.col("KCMMA") == "XX") | (F.col("KCMCA") == ""), F.lit(0))
        .otherwise(F.col("KCMCA").cast("double")))
)

CDT = ( 
    CDT.withColumn("passe_pro", F.when((F.col("KCMMA") > 10) & (F.col("KCMCA") < 15), "Licenciement_eco")
        .when(((F.col("KCMCA_num") >= 15) & (F.col("KCMCA_num") < 23)) |
          ((F.col("KCMCA_num") > 27) & (F.col("KCMCA_num") < 30)) |
          F.col("KCMCA_num").isin(25, 38, 50, 57, 91, 95, 80, 85), "Licenciement_autres")
        .when(F.col("KCMCA_num") == 40, "Fin_CDD")
        .when(F.col("KCMCA_num") == 41, "Fin_mission")
        .when(((F.col("KCMCA_num") > 59) & (F.col("KCMCA_num") < 80)) |
          ((F.col("KCMCA_num") > 99) & (F.col("KCMCA_num") < 176)) |
          F.col("KCMCA_num").isin(87, 45, 24), "Depart_volontaire")
        .when(F.col("KCMCA_num").isin(88, 92), "Rupture_conventionnelle")
        .when(F.col("KCMCA_num") == 1, "Manquant")
        .otherwise("Autres"))
)

date_ouvertures_droit = (
    PJC.withColumn("KDDPJ", F.to_date("KDDPJ"))
    .groupBy("id_midas", "KROD3")
    .agg(F.min("KDDPJ").alias("date_ouv_droit"))
    .filter((F.col("date_ouv_droit") >= F.lit(date_debut)) & (F.col("date_ouv_droit") <= F.lit(date_fin)))
    .withColumnRenamed("KROD3", "KROD1")
    .join("ODD", on=["id_midas", "KROD1"], how = "left")
    .select("id_midas", "KROD1", "date_ouv_droit", "KQISLP")
    .dropDuplicates()
)


fait_generateur = ( 
    DAL.join(date_ouvertures_droit, on=["id_midas", "KROD1"], how="right")
    .filter((F.col("KCNDDA") == 1) | (F.col("KCNDDA") == 3))
    .select("id_midas", "KROD1", "KCDA", "date_ouv_droit", "KQISLP")
    .dropDuplicates()
    .withColumnRenamed("KCDA", "KCDA1")
    .join(CDT, on=["id_midas", "KCDA1"], how="left")
    .select("id_midas", "KROD1", "KDFPE", "date_ouv_droit", "KQISLP", "passe_pro")
    .withColumn("KDFPE", F.to_date("KDFPE"))
    .dropDuplicates()
)


calcul_differe_specifique = (
    fait_generateur.withColumn("duree_differe_specifique", 
    F.when(F.col("passe_pro") == "Licenciement_eco", F.least(F.col("KQILSP") / coeff_2022, F.lit(75)))
    .when(F.col("passe_pro") == "Licenciement_autres", F.least(F.col("KQILSP") / coeff_2022, F.lit(150)))
    .otherwise(F.least(F.col("KQILSP") / coeff_2022, F.lit(150))))
)
```
:::
...
calcul_differe_CP = (
    ODD.select("id_midas", "KROD1", "KQICCP", "KQCSJP")
    .withColumn("differe_CP", F.least(F.col("KQICCP") / F.col("KQCSJP"), F.lit(30)))
    .select("id_midas", "KROD1", "differe_CP")
    .dropDuplicates()
)
...
fait_generateur = (
    DAL.join(calcul_differe_CP, on=["id_midas", "KROD1"], how="right")
    .filter((F.col("KCNDDA") == 1) | (F.col("KCNDDA") == 3))
    .select("id_midas", "KROD1", "KCDA", "differe_CP")
    .dropDuplicates()
    .withColumnRenamed("KCDA", "KCDA1")
    .join(CDT, on=["id_midas", "KCDA1"], how="left")
    .select("id_midas", "KROD1", "KDFPE", "differe_CP")
    .dropDuplicates()
    .withColumn("KDFPE", F.to_date("KDFPE"))
)
...
droits = PJC.select("id_midas", "KROD3").distinct()
...
date_fin_diff = (
    PJC
    .select("id_midas", "KROD3", "KDDPJ", "KDFPJ", "KCPJC", "KCMNI", "KCPJCC")
    .filter((F.col("KCPJC") == 3) & (F.col("KCMNI") == "00"))
    .join(droits, on=["id_midas", "KROD3"], how="right")
)

window_spec = Window.partitionBy("id_midas", "KROD3").orderBy("KDDPJ")

date_fin_diff = (
    date_fin_diff
    .withColumn("prev_KDFPJ", F.lag("KDFPJ").over(window_spec))
    .withColumn("ecart", F.datediff("KDDPJ", F.col("prev_KDFPJ")) - 1)
    .filter(F.col("ecart") > 0)
    .select("id_midas", "KROD3", "KDDPJ")
    .groupBy("id_midas", "KROD3")
    .agg(F.min("KDDPJ").alias("date_fin_diff"))
)
...
duree_differe_islr = (
    PJC
    .select("id_midas", "KROD3", "KDDPJ", "KDFPJ", "KCPJC", "KCMNI", "KCPJCC")
    .filter((F.col("KCPJC") == 3) & (F.col("KCMNI") == "00") & (F.col("KCPJCC") == "03"))
    .join(droits, on=["id_midas", "KROD3"], how="right")
    .join(date_fin_diff, on=["id_midas", "KROD3"], how="left")
    .withColumn("date_fin_diff", F.when(F.col("date_fin_diff").isNull(), F.max("KDFPJ").over(window_spec)).otherwise(F.col("date_fin_diff")))
    .filter(F.col("KDDPJ") < F.col("date_fin_diff"))
    .withColumn("duree_PJC", F.datediff("KDFPJ", "KDDPJ") + 1)
    .groupBy("id_midas", "KROD3")
    .agg(F.sum("duree_PJC").alias("differe_specifique"))
    .fillna({"differe_specifique": 0})
)
...
duree_differe_cp = (
    PJC
    .select("id_midas", "KROD3", "KDDPJ", "KDFPJ", "KCPJC", "KCMNI", "KCPJCC")
    .filter((F.col("KCPJC") == 3) & (F.col("KCMNI") == "00") & (F.col("KCPJCC") == "02"))
    .join(droits, on=["id_midas", "KROD3"], how="right")
    .join(date_fin_diff, on=["id_midas", "KROD3"], how="left")
    .withColumn("date_fin_diff", F.when(F.col("date_fin_diff").isNull(), F.max("KDFPJ").over(window_spec)).otherwise(F.col("date_fin_diff")))
    .filter(F.col("KDDPJ") < F.col("date_fin_diff"))
    .withColumn("duree_PJC", F.datediff("KDFPJ", "KDDPJ") + 1)
    .groupBy("id_midas", "KROD3")
    .agg(F.sum("duree_PJC").alias("differe_cp"))
    .fillna({"differe_cp": 0})
    .select("id_midas", "KROD3", "differe_cp")
)
...
duree_delai_attente = (
    PJC
    .select("id_midas", "KROD3", "KDDPJ", "KDFPJ", "KCPJC", "KCMNI", "KCPJCC")
    .filter((F.col("KCPJC") == 3) & (F.col("KCMNI") == "00") & (F.col("KCPJCC") == "04"))
    .join(droits, on=["id_midas", "KROD3"], how="right")
    .join(date_fin_diff, on=["id_midas", "KROD3"], how="left")
    .withColumn("date_fin_diff", F.when(F.col("date_fin_diff").isNull(), F.max("KDFPJ").over(window_spec)).otherwise(F.col("date_fin_diff")))
    .filter(F.col("KDDPJ") < F.col("date_fin_diff"))
    .withColumn("duree_PJC", F.datediff("KDFPJ", "KDDPJ") + 1)
    .groupBy("id_midas", "KROD3")
    .agg(F.sum("duree_PJC").alias("delai_attente"))
    .fillna({"delai_attente": 0})
    .select("id_midas", "KROD3", "delai_attente")
)
