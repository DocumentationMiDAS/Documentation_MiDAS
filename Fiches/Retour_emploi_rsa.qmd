...
spark = (SparkSession.builder
    ...
    .config("spark.sql.reduction.limit", "0")
...
chemin_cnaf = "hdfs:///dataset/MiDAS_v6/Allstat/minsoc/"
# Cohortes de BRSA considérées
cohortes = [f"{i:02d}22" for i in range(1, 13)]

# Chargement des tables prestations et filtre sur les cohortes mensuelles considérées
table_cnaf_totale_s = (spark.read
    .option("mergeSchema", "true")
    .parquet(f"{chemin_cnaf}cnaf_indiv_*.parquet")
    .select("id_midas", "RSAVERS", "MTRSAVER", "DTREFFRE")
    .withColumn("mois", to_date(col("DTREFFRE")))
    .filter((col("mois") >= "2022-01-01") & (col("mois") <= "2022-12-01"))
)
...
beneficiaires_rsa = (table_cnaf_totale_s
    .filter((col("RSAVERS").isin("RSA droit commun", "RSA droit local", "RSA jeune", "RSA expérimental", "C", "L", "J", "E")) & 
            (col("MTRSAVER") > 0))
    .select("id_midas", "mois")
    .dropDuplicates()
)

beneficiaires_rsa.createOrReplaceTempView("beneficiaires_rsa")

spark.catalog.cacheTable("beneficiaires_rsa")
...
chemin_mmo = "C:/Users/Public/Documents/MiDAS_parquet/Vague 6/MMO/"
MMO_2022 = spark.read.parquet(f"{chemin_mmo}MMO_2_2022_M6.parquet")
MMO_2023 = spark.read.parquet(f"{chemin_mmo}MMO_2_2023_M6.parquet")
MMO_2024 = spark.read.parquet(f"{chemin_mmo}MMO_2_2024_M6.parquet")

MMO_tot = MMO_2022.unionByName(MMO_2023).unionByName(MMO_2024)

MMO_tot = (MMO_tot
    .filter(F.col("Quali_Salaire_Base") == 7)
    .withColumn("FinCTT", F.to_date(F.col("FinCTT")))
    .withColumn("DebutCTT", F.to_date(F.col("DebutCTT")))
    .withColumn("type_contrat", F.when(F.col("Nature").isin("01", "09", "50", "82", "91"), "CDI")
                                .when(F.col("Nature").isin("02", "07", "10", "20", "21", "32", "51", "52", "80", "81", "92", "93"), "CDD")
                                .when(F.col("Nature").isin("03", "08"), "interim")
                                .when(F.col("Nature").isin("29", "53", "60", "70", "89", "90"), "autre")
                                .otherwise(None))
    .filter(F.col("DebutCTT").isNotNull())
    .withColumn("duree_ctt", F.datediff(F.col("FinCTT"), F.col("DebutCTT")) + F.lit(1))
    .withColumn("emploi_d", 
    F.when((F.col("type_contrat") == "CDD") & ((F.col("duree_ctt") > 180) | F.col("duree_ctt").isNull()), 1)
    .when(F.col("type_contrat") == "CDI", 1).otherwise(0))
    .select("id_midas", "DebutCTT", "FinCTT", *[c for c in MMO_tot.columns if c.startswith("emploi_bit")], "emploi_d")
)
...
MMO_pivot = (MMO_tot
    ...
    *[F.struct(F.lit(c.replace("emploi_bit_", "")).alias("date_emploi_bit"),
               F.col(c).alias("emploi_bit")).alias(c) for c in MMO_tot.columns if c.startswith("emploi_bit_")])
    .selectExpr("id_midas", "DebutCTT", "FinCTT", "stack(" + 
             str(len([c for c in MMO_tot.columns if c.startswith("emploi_bit_")])) + ", " +
             ", ".join([f"'{c}', {c}" for c in MMO_tot.columns if c.startswith("emploi_bit_")]) + 
             ") as (date_emploi_bit, emploi_bit)")
)
...
MMO_pivot = (MMO_pivot
    .withColumn("date_emploi_bit_deb", F.to_date(F.concat_ws("-", F.substring("date_emploi_bit", 1, 4), F.substring("date_emploi_bit", 6, 2), F.lit("01"))))
    .withColumn("date_emploi_bit_fin", F.last_day(F.col("date_emploi_bit_deb")))
    .filter(F.col("emploi_bit") == 1) # on ne conserve que les lignes contrats X mois où les contrats sont en emploi BIT
)
...
# Table des horizons qu'on souhaite considérer
# En PySpark, on crée directement le DataFrame sans passer par une copie
horizon_num = spark.createDataFrame([Row(horizon=i) for i in range(25)])

# Produit cartésien de chaque bénéficiaire du RSA un mois donné avec les 24 horizons observés
horizon_brsa = beneficiaires_rsa.crossJoin(horizon_num)

# Calcul du mois calendaire d'horizon : si on considère un BRSA en janvier 2022, l'horizon 2 est mars 2022
horizon_brsa = horizon_brsa.withColumn("date_horizon", F.expr("add_months(mois, horizon)"))
...
# Jointure avec les mois d'emploi BIT
ajout_mois_emploi_bit = horizon_brsa.alias("LHS")
    .join(MMO_pivot.alias("RHS"),
    (F.col("LHS.id_midas") == F.col("RHS.id_midas")) &
    (F.col("RHS.date_emploi_bit_deb") <= F.col("LHS.date_horizon")) &
    (F.col("RHS.date_emploi_bit_fin") >= F.col("LHS.date_horizon")),
    how="left")

ajout_mois_emploi_bit = (ajout_mois_emploi_bit
    .withColumn("en_emploi", F.when(F.col("DebutCTT").isNotNull(), 1).otherwise(0)) # si un contrat est retrouvé, l'individu est en emploi salarié le mois d'horizon considéré
    .withColumn("en_emploi_durable", F.when((F.col("DebutCTT").isNotNull()) & (F.col("emploi_d") == 1), 1).otherwise(0))
)

ajout_mois_emploi_bit = (ajout_mois_emploi_bit.groupBy("id_midas", "KROD3", "horizon") # on dédoublonne : plusieurs contrats emploi BIT peuvent être retrouvés pour un individu un même mois d'horizon
    .agg(
        F.when(F.sum("en_emploi") >= 1, 1).otherwise(0).alias("en_emploi"),
        F.when(F.sum("en_emploi_durable") >= 1, 1).otherwise(0).alias("en_emploi_durable")
    )
)

ajout_mois_emploi_bit.createOrReplaceTempView("ajout_mois_emploi_bit")

spark.catalog.cacheTable("ajout_mois_emploi_bit")

# Ajout à la table individuelle de bénéficiaires mensuels du RSA
table_finale = (beneficiaires_rsa.select("id_midas", "KROD3", "mois").distinct()
    .join(ajout_mois_emploi_bit, on=["id_midas", "KROD3"], how="left")
)
...
prop_re = (
    table_finale
    .groupBy("mois", "horizon")
    .agg(
        F.sum("en_emploi").alias("eff"),
        F.count("*").alias("tot")
    )
    .withColumn("prop", (F.col("eff") * 100 / F.col("tot")))
    .orderBy("mois")
)

mois = prop_re.select("mois").rdd.flatMap(lambda x: x).collect()

# La suite n'est pas proposée en PySpark car elle utilise ggplot de R
